"""Luma 3-min — Recipe 2 minimal + kick-locked vocals (approve before 35-min)."""
...
SLUG = "luma"
# Short on-kick name call (same pocket as يا لمى / يا ليل).
HOOK = "يا لوما"
BPM = 104
SEED = 100480
...
VOCAL_GRID = (
    "VOCALS (MANDATORY): every syllable of 'يا لوما' lands ON the kick. "
...
def main() -> None:
    """Generate Luma Recipe 2 3-min for listen/approval."""
    out_dir = OUTPUT_DIR / SLUG
    sources = OUTPUT_DIR / "_recipe2_sources"
    out_dir.mkdir(parents=True, exist_ok=True)
    sources.mkdir(parents=True, exist_ok=True)

    # Keep old 35min aside — do not treat it as the source of truth.
    old35 = out_dir / f"{SLUG}_35min.mp3"
    if old35.is_file():
        archived = sources / f"{SLUG}_35min_old_messy.mp3"
        if not archived.is_file():
            shutil.copy2(old35, archived)
            print(f"archived old 35min → {archived.name}")
    ...
    payload = build_recipe2_payload(
        ...
        bpm=BPM,
        color_note=(
            f"لوما — {MINIMAL} {SHORT_INTRO} {VOCAL_GRID} "
            "intimate close-mic Arabic female, warm soft light night mood, "
    ...
    print(f"GENERATE luma 3min Recipe2 minimal+grid seed={SEED}")
    meta_gen = generate_to_file(
        payload,
        api_base=API_BASE,
        api_key="",
        out_path=raw,
        label="luma_3min_recipe2",
    )
    ...
    info = {
        ...
        "seed": SEED,
        "recipe": "haya_recipe2 — minimal + vocal grid (Lama/Layl lane)",
