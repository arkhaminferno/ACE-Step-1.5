"""Generate Amira 3-min — Recipe3 trio: piano intro, oud chorus, ney color."""
...
def _cut_cues(final: Path, cues: Path) -> None:
    """Write section cues for A/B (no autoplay)."""
...
def main() -> int:
    """Generate brand-new Amira with piano→oud→ney section map."""
    ...
    payload = build_recipe3_payload(
        ...
        lead=LEAD,
        color_note=(
            "ORIGINAL Amira — piano intro, oud instrumental heart, "
            "sparse ney outro color; NOT a HAYA remix"
        ),
    )
    print(f"GENERATE {SLUG} lead={LEAD} seed={SEED} hook={HOOK}", flush=True)
    meta = generate_to_file(
        payload, api_base=API_BASE, api_key="", out_path=raw, label=f"{SLUG}-trio"
    )
    ...
    info = {
        "slug": SLUG,
        "lead": LEAD,
        "map": "intro=piano, chorus=oud, outro/break=ney",
        "hook": HOOK,
        "seed": SEED,
