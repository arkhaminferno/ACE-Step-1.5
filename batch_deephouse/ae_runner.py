"""Run After Effects JSX and aerender for one HAYA render job."""
...
from batch_deephouse.ae_config import resolve_aerender
from batch_deephouse.ae_headless import launch_jsx, quit_ae
from batch_deephouse.ae_launcher import ACTIVE_SCRIPT, write_active_script
from batch_deephouse.ae_render_job import RenderJob, write_render_job
...
def run_jsx_script(
    ...
    *,
    ...
    job_dict = job.to_dict() if job is not None else None
    write_active_script(body_script_name, job=job_dict)
    run_active_script(expect_project=expect_project, timeout_sec=timeout_sec)


def run_aerender(job: RenderJob, *, timeout_sec: int = 14400) -> Path:
    """Render the saved per-slug project to MP4 (long mixes need a high timeout)."""
